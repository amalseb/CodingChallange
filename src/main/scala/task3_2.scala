...
object Task3_2 {
  def main(args: Array[String]): Unit = {
    ...
    val spark = Common.getSparkSession(Constants.TASK3_2)
    ...
    //Predict on the given new data
     val newData = spark.createDataFrame(Seq((5.1, 3.5, 1.4, 0.2), (6.2, 3.4, 5.4, 2.3)))
      .toDF(Constants.SEPAL_LENGTH, Constants.SEPAL_WIDTH, Constants.PETAL_LENGTH, Constants.PETAL_WIDTH)

     val newDataWithFeatures = assembler.transform(newData)

     val newPrediction = lrModel.transform(newDataWithFeatures)

     val newPredictedWithClassNames = newPrediction
    ...
    newPredictedWithClassNames.select(Constants.CLASS_NAME)

    Common.writeAsTextFile(Constants.TASK3_2_FILEPATH, newPredictedWithClassNames.withColumn(Constants.CLASS,col(Constants.CLASS_NAME)).select(Constants.CLASS))
